...
// OID for id-aa-signatureTimeStampToken (RFC 3161 / RFC 5126), embedded as a CMS
// unsigned attribute. PdfSignerCms here has no dedicated timestamp-token setter, so
...
std::string PoDoFo::PdfRemoteSignDocumentSession::beginSigning() {
    try {
        ...
        // and a DSS dictionary (B_LT and above) get attached, handled separately below.
        if (_conformanceLevel != "ADES_B_B" && _conformanceLevel != "ADES_B_T" &&
            _conformanceLevel != "ADES_B_LT" && _conformanceLevel != "ADES_B_LTA") {
        ...
        _signer = std::make_shared<PoDoFo::PdfSignerCms>(cert, _cmsParams);
        _signer->ReserveAttributeSize(20000);
        for (const auto& chainCert : _certificateChainDer) {
            _signer->AddCertificate({ reinterpret_cast<const char*>(chainCert.data()), chainCert.size() });
        }
...
void PoDoFo::PdfRemoteSignDocumentSession::finishSigning(const std::string& signedHash, const std::string& base64Tsr, const std::optional<ValidationData>& validationData) {
    try {
        ...
        if (_conformanceLevel != "ADES_B_B") {
            ...
            _signer->AddAttribute(SIGNATURE_TIME_STAMP_TOKEN_OID,
                { timestampToken.data(), timestampToken.size() });
        }
...
void PoDoFo::PdfRemoteSignDocumentSession::setTimestampToken(const std::string& responseTsrBase64) {
    _responseTsrBase64 = responseTsrBase64;
}
